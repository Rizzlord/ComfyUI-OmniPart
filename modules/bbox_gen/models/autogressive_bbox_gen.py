...
class BboxGen(nn.Module):
    ...
    def _load_partfield_encoder(self):
        """
        Loads the PartField encoder model using its self-contained configuration and setup system.
        """
        # 1. Dynamically import the necessary PartField modules
        ...
        # 2. Build the configuration for the PartField model, telling it which checkpoint to use.
        ...
        # 3. Instantiate the model. The PartField's `Model` class handles its own weight loading.
        #    This single line correctly creates and loads the pretrained encoder.
        self.partfield_encoder = Model(cfg)
        self.partfield_encoder.eval()

        # 4. Set parameters to not require gradients
        for param in self.partfield_encoder.parameters():
            param.requires_grad = False
            
        print("PartField encoder loaded successfully.")
